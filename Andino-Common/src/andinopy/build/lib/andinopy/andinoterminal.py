...
class AndinoTerminal:
    class RfidKeyboardHandle:
        _interruptPin = 23
        _slaveAddress = 0x4
        _i2c = smbus2.SMBus(1)
    ...
    # Part instances
    andinoio_instance: andinoio
    display_instance: DisplayHandle
    rfid_keyboard_instance: RfidKeyboardHandle

    def __init__(self,
                 andinoio_instance: andinoio = None,
                 display_instance: DisplayHandle = None,
                 rfid_keyboard_instance: RfidKeyboardHandle = None):
        """
        Create a new Terminal instance
        Can be passed preconfigured handles
        """
        log.debug("Terminal starting initialization")
        self.andinoio_instance = andinoio_instance if andinoio_instance is not None else andinoio()
        self.display_instance = display_instance if display_instance is not None else self.DisplayHandle()
        self.rfid_keyboard_instance = rfid_keyboard_instance \
            if rfid_keyboard_instance is not None else self.RfidKeyboardHandle()
