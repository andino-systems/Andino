...
// INFO           ( print settings)
// HARD           ( Hardware, 0=noShield, 1=1DI2DO, 2=3DI, 3=5DI, 4=DS18B20 board)
...
// SENDT 8000     ( send temperature reports every xxx ms )
...
// ADDRT 1|2      ( show addresses of all temperature sensors connected to bus x )
// TBUS 1|2       ( sets the number of temperature sensor busses to 1 or 2 )
// POLLT          ( prints a single status message for all active temperature busses )
//
...
// Data wire port. Note that this is the Arduino port, not the Atmel Controller port. Port 11 is equivalent to PB3 on the microcontroller. For a full overview, view https://raw.githubusercontent.com/andino-systems/Andino/master/Andino-X1/BaseBoard/Pinout-UNOrev3_latest.png
...
struct Setup
{
  ...
  unsigned int  SendCycleT;   // Send cycle of the temperature information in ms
  byte          Shield;       // 0 = none, 1 = 1DI2DO, 2 = 3DI, 3 = 5DI, 4 = DS18B20
  ...
  byte          BusNr;         // 1 for one Bus (only Pin 11/PB3), 2 for two busses (Pin 11/PB3 and 12/PB4)
} TheSetup;
...
void setup() 
{
  ...
  setup_in_out();
  if(TheSetup.Shield == SHIELD_DS18B20){
    setup_onewire();
  }
  setup_interrupt();
}
...
void setup_onewire()
{
  ...
  //printAddress(Thermometer, sensors, 1);
  
  if(TheSetup.BusNr==2)
  {
    oneWire2=OneWire(ONE_WIRE_BUS_2);
    sensors2=DallasTemperature(&oneWire2);
    sensors2.begin();
    sensorCount2 = sensors2.getDeviceCount();
    //printAddress(Thermometer2, sensors2, 2);
  }
}

void printAddress(DeviceAddress deviceAddress, DallasTemperature sens, int busId)
{ 
  Serial.print(":");
  ...
  int j=0;
    ...
    first = (loopCounter >> 12) | 48;
    if (first > 57) tmp[0] = first + (byte)7;
    else tmp[0] = first ;
  
    first = ((+loopCounter>>8) & 0x0F) | 48;
    if (first > 57) tmp[1] = first + (byte)7; 
    else tmp[1] = first;

    first = ((+loopCounter>>4) & 0x0F) | 48;
    if (first > 57) tmp[2] = first + (byte)7; 
    else tmp[2] = first;

    first = (loopCounter & 0x0F) | 48;
  ...
  Serial.print(busId);
  Serial.print("}");
  for (int i = 0;  i < sens.getDeviceCount();  i++)
  {
    Serial.print("{");
    Serial.print(i);
    Serial.print("*");
...
void setup_in_out()
{
  ...
  if( TheSetup.Shield == SHIELD_3DI || TheSetup.Shield == SHIELD_5DI || TheSetup.Shield == SHIELD_DS18B20)
  {  
    pinMode(IN_4,INPUT); 
  ...
  if( TheSetup.Shield == SHIELD_5DI || TheSetup.Shield == SHIELD_DS18B20)
  {  
    pinMode(IN_6,INPUT); 
...
unsigned long lastSendMillisT = 0;
...
void loop() 
{
  ...
  if(   ((currentMillis - lastSendMillis  >= TheSetup.SendCycle)&&(TheSetup.SendCycle != 0)) 
  ...
  //OneWire loop section
  if((TheSetup.Shield == SHIELD_DS18B20)&&(currentMillis - lastSendMillisT  >= TheSetup.SendCycleT)&&((TheSetup.SendCycleT != 0)||sendTSingle))
  {
    lastSendMillisT = currentMillis;
    // Requesting temperatures from all devices
    sensors.requestTemperatures();
    if(TheSetup.BusNr==2){
      sensors2.requestTemperatures();
    }    
    constructTempMsg(sensors, sensorCount, 1);
    if(TheSetup.BusNr==2){
      constructTempMsg(sensors2, sensorCount2, 2);
    }
    sendTSingle = false;
  }
}

void constructTempMsg(DallasTemperature sens, int sensCount, int busId){
  // Constructing serial output message
    float temps [sensCount] = { };
    String msg = "";
    Serial.print(":");
    PrintHex16(++loopCounter);
    Serial.print("@T{");
    ...
    for (int i = 0; i < sensCount; i++) {
      temps [i] = sens.getTempCByIndex(i);
...
void timerInterrupt()
{
  ...
  if( ( TheSetup.Shield == SHIELD_NONE ) || ( TheSetup.Shield == SHIELD_DS18B20) )
    return;
  doCounter( &Counter3, bitRead( IN_3_PORT, IN_3_PIN ) );
  if( TheSetup.Shield == SHIELD_1DI2DO )
    return;
  // only read inputs 4 and 5 if SendCycleT is 0 to prevent possible conflicts
...
void SetupDefault()
{
  ...
  TheSetup.SendCycleT = 0;    // Cycle in ms
  ...
  TheSetup.BusNr = 1;         // One Temperature Sensor Bus
}
...
void OnDataReceived()
{
  ...
  if( cmd.startsWith("POLLT") && (TheSetup.Shield == SHIELD_DS18B20))
    {
      if(TheSetup.SendCycleT != 0){
        Serial.println("ERROR. SENDT NOT 0");
      } else {
        Serial.println( "POLLT");
      }
      sendTSingle = true;
      result = true;
    }
    
  unsigned int value = toInt( cmd, 5 );
  if( value != -1 )
  {
    ...
    if( cmd.startsWith("TBUS "))
    {
      if( !checkRange( value, 1, 2 ) && (TheSetup.Shield == SHIELD_DS18B20))
        return;
      Serial.print( "TBUS ");
      Serial.println( value, DEC );
      writeSetup = TheSetup.BusNr != value;
      TheSetup.BusNr = value;
      result = true;
    }
    else
    if( cmd.startsWith("ADDRT ") && (TheSetup.Shield == SHIELD_DS18B20))
    {
      if( !checkRange( value, 1, 2 ) )
        return;
      Serial.print( "ADDRT ");
      Serial.println( value, DEC );
      if(value==1)
      ...
      {
        printAddress(Thermometer, sensors2, 2);
      }
      result = true;
    }
    else
    if( cmd.startsWith("SEND "))
    {
      if(value == 0){
        ...
        TheSetup.SendCycle = 0;
        result = true;
      }
      else{
        if( !checkRange( value, 100, 60000 ) )
        return;
      Serial.print( "SEND ");
      Serial.println( value, DEC );
      writeSetup = TheSetup.SendCycle != value;
    ...
    if( cmd.startsWith("SENDT "))
    {
      if(value == 0){
        Serial.print( "SENDT ");
        Serial.println( value, DEC );
        writeSetup = TheSetup.SendCycleT != value;
        TheSetup.SendCycleT = 0;
        result = true;
      }
      else{
        if( !checkRange( value, 2000, 60000 ) )
        return;
        Serial.print( "SENDT ");
        Serial.println( value, DEC );
        writeSetup = TheSetup.SendCycleT != value;
        TheSetup.SendCycleT = value;
        result = true;
      }
    }
  }
...
void DoCmdInfo()
{
    ...
    Serial.print( "TBUS "); Serial.println( TheSetup.BusNr, DEC );
    ...
    Serial.print( "SENDT "); Serial.println( TheSetup.SendCycleT, DEC);
    ...
    Serial.println( "HARD 4 (DS18B20 board)" );
    Serial.println( "POLLT" );
...
void DoCmdReset()
{
  // The reset feature is currently disabled due to a bug in the Arduino bootloader that causese the system to enter a reset loop. As a workaround, restart the serial connection to reset the board.
  Serial.println("NOT IMPLEMENTED ERROR");
  /*
  delay(500);
  wdt_disable();
