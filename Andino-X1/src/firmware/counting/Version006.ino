...
struct Setup
{
  ...
  byte          BusNr;         // 1 for one Bus (only Pin 11), 2 for two busses (Pin 11 and 12)
} TheSetup;
...
void loop() 
{
     ...
     || (TheSetup.SendOnChange && pinChanged ) )
  {
    ...
    if( TheSetup.SendCounter )
    {
      ...
      if( TheSetup.Shield == SHIELD_1DI2DO || TheSetup.Shield == SHIELD_3DI || TheSetup.Shield == SHIELD_5DI || TheSetup.Shield == SHIELD_DS18B20)
      {
        Serial.write( ',' );
